...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `k1` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@k1tools/shared/git`, `@k1tools/shared/DrainableWorker`) — no barrel index.
