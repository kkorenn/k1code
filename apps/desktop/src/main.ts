...
function backendChildEnv(): NodeJS.ProcessEnv {
  ...
  delete env.K1CODE_PORT;
  delete env.K1CODE_AUTH_TOKEN;
  delete env.K1CODE_MODE;
  delete env.K1CODE_NO_BROWSER;
  delete env.K1CODE_HOST;
  delete env.K1CODE_DESKTOP_WS_URL;
  return env;
}
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { k1codeCommitHash?: unknown };
    return normalizeCommitHash(parsed.k1codeCommitHash);
  } catch {
    return null;
  }
}
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.K1CODE_COMMIT_HASH);
  if (envCommitHash) {
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("K1 Code failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
function handleCheckForUpdatesMenuClick(): void {
  const disabledReason = getAutoUpdateDisabledReason({
    ...
    disabledByEnv: process.env.K1CODE_DISABLE_AUTO_UPDATE === "1",
  });
  if (disabledReason) {
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `K1 Code ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
...
function configureAutoUpdater(): void {
  ...
  const githubToken =
    process.env.K1CODE_DESKTOP_UPDATE_GITHUB_TOKEN?.trim() || process.env.GH_TOKEN?.trim() || "";
  if (githubToken) {
