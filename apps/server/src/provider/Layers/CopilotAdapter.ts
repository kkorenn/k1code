 ...
 *
 * Runs Copilot Agent CLI in non-interactive mode per turn and projects outputs
...
const DEFAULT_COPILOT_BINARY_PATH = "copilot";
  ...
  Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        if (input.provider !== undefined && input.provider !== PROVIDER) {
          return yield* new ProviderAdapterValidationError({
            provider: PROVIDER,
            operation: "CopilotAdapter.startSession",
        ...
        const binaryPath =
          input.providerOptions?.copilot?.binaryPath?.trim() ||
          existing?.binaryPath ||
          DEFAULT_COPILOT_BINARY_PATH;
        const now = nowIso();
        const model = input.model?.trim() || existing?.session.model || DEFAULT_COPILOT_MODEL;
      ...
      Effect.tryPromise({
        try: () =>
              ...
              finish(() => {
                reject(
                  new ProviderAdapterRequestError({
                    provider: PROVIDER,
                    method: "CopilotAdapter.sendTurn",
                    detail: toMessage(error, "Failed to spawn Copilot CLI process."),
                    cause: error,
                  }),
                );
              });
          ...
          new ProviderAdapterRequestError({
            provider: PROVIDER,
            method: "CopilotAdapter.sendTurn",
            detail: toMessage(cause, "Failed to run Copilot CLI command."),
            cause,
          }),
      });
    ...
    const runTurn = (input: {
      ...
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          const model =
            input.turnInput.model?.trim() || input.context.session.model || DEFAULT_COPILOT_MODEL;
          const binaryPath = input.context.binaryPath;
          const args = ["-p", prompt, "--force", "--output-format", "text", "--model", model];

          yield* logNativeEvent(
            {
              type: "copilot.turn.command",
              threadId,
              turnId: input.turnId,
              binaryPath,
              cwd,
          ...
          if (!runningTurn || runningTurn.turnId !== input.turnId) {
            return yield* new ProviderAdapterRequestError({
              provider: PROVIDER,
              method: "CopilotAdapter.sendTurn",
              detail: "Turn lifecycle state was lost before Copilot process startup.",
            });
          }
          ...
          if (execution.code !== 0) {
            const rawMessage =
              ...
              `Copilot CLI exited with code ${execution.code ?? "unknown"}.`;
          ...
          Effect.matchEffect({
            onFailure: (error) =>
              Effect.gen(function* () {
                const rawMessage = toMessage(error, "Copilot turn failed.");
                const message = normalizeCursorFailureMessage(rawMessage);
                const finishAt = nowIso();
                yield* emitRuntimeEvent({
                  type: "runtime.error",
      ...
      Effect.gen(function* () {
        ...
        if (context.runningTurn) {
          return yield* new ProviderAdapterRequestError({
            provider: PROVIDER,
            method: "CopilotAdapter.sendTurn",
            detail: "A Copilot turn is already running for this thread.",
          });
        }

        const turnId = TurnId.makeUnsafe(` copilot -turn-${crypto.randomUUID()}`);
        const itemId = RuntimeItemId.makeUnsafe(` copilot -item-${crypto.randomUUID()}`);
    ...
    const respondToRequest: CopilotAdapterShape["respondToRequest"] = (
      ...
      Effect.fail(
        new ProviderAdapterRequestError({
          provider: PROVIDER,
          method: "CopilotAdapter.respondToRequest",
          detail: `Copilot has no pending approval request '${requestId}' for thread '${threadId}'.`,
          cause: { decision },
        }),
      );
    ...
    const respondToUserInput: CopilotAdapterShape["respondToUserInput"] = (
      ...
      Effect.fail(
        new ProviderAdapterRequestError({
          provider: PROVIDER,
          method: "CopilotAdapter.respondToUserInput",
          detail: `Copilot has no pending user-input request '${requestId}' for thread '${threadId}'.`,
          cause: { answers },
        }),
      );
    ...
    const rollbackThread: CopilotAdapterShape["rollbackThread"] = (threadId, numTurns) =>
      Effect.gen(function* () {
        ...
        if (context.runningTurn) {
          return yield* new ProviderAdapterRequestError({
            provider: PROVIDER,
            method: "CopilotAdapter.rollbackThread",
            detail: "Cannot roll back while a Copilot turn is running.",
          });
        }
