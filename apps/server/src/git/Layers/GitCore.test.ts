...
const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), { prefix: "k1-git-core-test-" });
  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "k1code/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "k1code/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "k1code/tmp-working" });

        const renamed = yield* (yield* GitCore).renameBranch({
          cwd: tmp,
          oldBranch: "k1code/tmp-working",
          newBranch: "k1code/feat/session",
        });

        expect(renamed.branch).toBe("k1code/feat/session-1");
        const branches = yield* (yield* GitCore).listBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "k1code/feat/session")).toBe(
          true,
        );
        expect(branches.branches.some((branch) => branch.name === "k1code/feat/session-1")).toBe(
          true,
        );
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("k1code/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "k1code/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "k1code/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "k1code/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "k1code/tmp-working" });
        ...
        expect(renamed.branch).toBe("k1code/feat/session-2");
      }),
    );
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, ["remote", "add", "origin", "git@github.com:kkorenn/k1code.git"]);

          const remoteName = yield* core.ensureRemote({
            cwd: tmp,
            preferredName: "origin",
            url: "git@github.com:kkorenn/k1code.git/",
          });
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(
            yield* git(tmp, ["ls-remote", "--heads", "jasonLaster", "k1code/pr-488/statemachine"]),
          ).toBe("");
        }),
    );
