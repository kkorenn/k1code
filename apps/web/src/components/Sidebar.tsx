...
function K1Wordmark() {
  return (
    <span aria-label="K1" className="shrink-0 text-base font-semibold leading-none text-foreground">
      K1
    </span>
  );
}
...
export default function Sidebar() {
  ...
  const sortedProjects = useMemo(
    () => sortProjectsForSidebar(projects, unarchivedThreads, appSettings.sidebarProjectSortOrder),
    [appSettings.sidebarProjectSortOrder, projects, unarchivedThreads],
  );
  ...
  function renderProjectItem(
    ...
    const projectThreads = sortThreadsForSidebar(
      unarchivedThreads.filter((thread) => thread.projectId === project.id),
      appSettings.sidebarThreadSortOrder,
    );
